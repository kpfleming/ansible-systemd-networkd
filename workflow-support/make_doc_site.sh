...
rm -f "${basedir}/src/kpfleming-systemd_networkd-0.0.0.tar.gz"
...
build_cmd ansible-galaxy collection install "src/kpfleming-systemd_networkd-0.0.0.tar.gz"

build_cmd mkdir docs-build

build_cmd antsibull-docs sphinx-init --fail-on-error --use-current --dest-dir docs-build kpfleming.systemd_networkd
